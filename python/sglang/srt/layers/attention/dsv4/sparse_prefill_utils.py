...
def build_swa_token_ids(
    ...
    zero_initialize_tail: bool = False,
    ...
    allocator = torch.zeros if zero_initialize_tail else torch.empty
    swa_token_ids = allocator(total_swa, dtype=torch.int32, device=device)
    if total_swa == 0:
...
class SparsePrefillChunkCache:
    ...
    max_seq_len: int
    # True when all geometry-producing kernels are recorded at the beginning
    # of the strict Huge CUDA Graph. Replay then preserves this object's tensor
    # addresses and does not rebuild/copy a temporary host-orchestrated cache.
    graph_local_rebuild: bool
    ...
    # Capture-stable descriptor inputs. Breakable-CG replay constructs a new
    # ForwardBatch wrapper, so directly capturing its tensor addresses would
    # leave the graph reading the dummy capture batch forever. Replay refreshes
    # these tiny buffers in place before launching the graph.
    extend_seq_lens: torch.Tensor  # (num_reqs,) int32
    req_pool_indices: torch.Tensor  # (num_reqs,) int32
    ...
    def build(
        ...
        max_seq_len: int,
        total_swa_hint: Optional[int] = None,
        zero_initialize_swa_tail: bool = False,
        ...
        swa_token_ids, swa_first_pos, swa_gather_lens, swa_offsets = (
            build_swa_token_ids(
                ...
                zero_initialize_tail=zero_initialize_swa_tail,
            )
        )
    ...
    def refresh_graph_inputs_(
        ...
        *,
        ...
        """Update only the capture-stable request descriptor inputs.

        Geometry outputs remain graph-owned and are regenerated by the first
        kernels in the monolithic Huge graph. This deliberately avoids the
        old path that rebuilt every sparse tensor eagerly and copied all of
        them into capture allocations before replay.
        """
        ...
        for name, incoming in (
            ...
            captured = getattr(self, name)
            incoming = incoming.to(dtype=torch.int32, device=captured.device)
            if captured.shape != incoming.shape:
                raise RuntimeError(
                    "Huge Graph request descriptor shape changed: "
                    f"{name}={tuple(captured.shape)} vs {tuple(incoming.shape)}"
                )
            captured.copy_(incoming)

    def rebuild_swa_c0_in_graph_(
        self,
        *,
        req_to_token: torch.Tensor,
        full_to_swa: torch.Tensor,
    ) -> None:
        """Regenerate SWA/C0 geometry directly into pre-capture buffers.

        All destination tensors belong to the metadata object allocated before
        CUDA Graph capture. Only short-lived scalar/arange inputs may use the
        graph pool; no graph-private output survives into decoder layers.
        """
        ...
        torch.cumsum(
            ...
            dim=0,
            out=self.query_start_loc[1:],
        )
        torch.minimum(
            self.seq_lens,
            self.extend_seq_lens + (self.swa_window_size - 1),
            out=self.swa_gather_lens,
        )
        torch.sub(
            self.seq_lens,
            self.swa_gather_lens,
            out=self.swa_first_pos,
        )
        self.swa_offsets[0].zero_()
        torch.cumsum(
            self.swa_gather_lens,
            dim=0,
            out=self.swa_offsets[1:],
        )
        # The strict graph bucket allocates M+W-1 entries even for capture
        # dummy chunks whose live union is shorter. Keep the unused tail safe.
        self.swa_token_ids.zero_()
        _build_swa_token_ids_kernel[(self.num_reqs, 128)](
            self.swa_token_ids,
            self.swa_first_pos,
            self.swa_gather_lens,
            self.swa_offsets,
        ...
        self.c0_combined_indices.fill_(-1)
        zero_topk = torch.zeros(
            (self.num_qo_tokens, 1),
            dtype=torch.int32,
        ...
        combine_topk_swa_indices(
            ...
            compressed_base=zero_compressed_base,
            swa_base=self.swa_offsets[:-1],
            window_size=self.swa_window_size,
            compress_ratio=1,
            topk=0,
            out_indices=self.c0_combined_indices,
            out_lens=self.c0_combined_lens,
        )

    def rebuild_c4_in_graph_(self, *, page_table: torch.Tensor) -> None:
        """Regenerate C4 gather geometry directly into stable graph buffers."""

        if (
            self.c4_flat_token_ids is None
            or self.c4_compressed_base is None
            or self.c4_swa_base is None
            or self.c4_page_size is None
        ):
            raise RuntimeError("C4 graph buffers must be allocated before capture")
        c4_max = self.c4_flat_token_ids.shape[0] // self.num_reqs
        block_size = 256
        _build_c4_geometry_kernel[
            (self.num_reqs, triton.cdiv(c4_max, block_size))
        ](
            self.c4_flat_token_ids,
        ...
        # The fused per-layer top-k epilogue only overwrites valid prefixes.
        self.c4_combined_indices.fill_(-1)
        self.c4_combined_lens.zero_()

    def rebuild_c128_for_graph_replay_(
        self,
        *,
        ...
        C128 graph-local writes were not end-to-end safe on the current
        Breakable-CG allocator. SWA/C0/C4 are overwritten inside the graph, so
        ...
        if (
            ...
            or self.c128_combined_lens is None
        ):
        ...
        _rebuild_c128_replay_kernel[(self.num_reqs, 128)](
            ...
            C128_MAX=c128_max,
            COMBINED_WIDTH=self.c128_combined_indices.shape[1],
            PADDED_COMBINED=triton.next_power_of_2(
    ...
    def refresh_for_breakable_cuda_graph_replay_(
        self, other: "SparsePrefillChunkCache"
    ) -> None:
        """Refresh live geometry without changing capture-time addresses."""
        ...
        for name in scalar_fields:
            ...
            if current != incoming:
                raise RuntimeError(
                    "breakable DSV4 sparse-cache shape changed at replay: "
        ...
        for name in tensor_fields:
            ...
            if dst is None or src is None:
                raise RuntimeError(
                    "breakable DSV4 sparse-cache allocation mismatch: "
                    f"{name}: captured={type(dst).__name__}, "
                    f"live={type(src).__name__}"
                )
            if dst.shape != src.shape or dst.dtype != src.dtype:
                raise RuntimeError(
                    "breakable DSV4 sparse-cache tensor changed at replay: "
