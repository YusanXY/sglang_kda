...
def _jit_tp4_moe_mhc_post_module(use_pdl: bool) -> Module:
    args = make_cpp_args(use_pdl)
    return load_jit(
        make_name("tp4_moe_mhc_post_hc4_h4096_owner_v41a2_t4_interleaved"),
        *args,
        ...
        cuda_wrappers=[
            ...
            (
                "run_stream_signal",
                f"Tp4MoeMhcPostKernel<{args}>::run_stream_signal",
            ),
        ],
        extra_cuda_cflags=["--use_fast_math"],
        extra_ldflags=["-lcuda"],
...
@register_custom_op(
    op_name="dsv4_tp4_stream_mem_signal",
    mutates_args=["local_sync"],
)
def _tp4_stream_mem_signal_custom_op(
    local_sync: torch.Tensor,
    slot: int,
    epoch: int,
) -> None:
    module = _jit_tp4_moe_mhc_post_module(False)
    module.run_stream_signal(local_sync, slot, epoch)
...
def tp4_moe_owner_mhc_post(
    ...
    if len(owners) != 4 or len(syncs) != 4:
        raise RuntimeError(
            "TP4 owner mHC post requires four owner buffers and four sync peers"
        )
    m = residual.shape[0]
    expected = (
    ...
    for sync in syncs:
        if (
            ...
            raise RuntimeError(
                "TP4 owner mHC post requires contiguous CUDA int32[256] peers"
            )
    if sync_slot not in range(256) or not 1 <= sync_epoch <= 0x7FFFFFFF:
        raise RuntimeError("invalid TP4 owner mHC post sync slot or epoch")
...
@debug_kernel_api
def tp4_stream_mem_signal(
    local_sync: torch.Tensor,
    slot: int,
    epoch: int,
) -> None:
    """Publish owner readiness after prior stream writes, without a kernel."""

    if (
        local_sync.shape != (256,)
        or local_sync.dtype != torch.int32
        or local_sync.device.type != "cuda"
        or not local_sync.is_contiguous()
    ):
        raise RuntimeError(
            "TP4 stream signal requires contiguous CUDA int32[256] storage"
        )
    if slot not in range(256) or not 1 <= epoch <= 0x7FFFFFFF:
        raise RuntimeError("invalid TP4 stream signal slot or epoch")
    _tp4_stream_mem_signal_custom_op(local_sync, slot, epoch)
